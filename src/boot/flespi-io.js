...
async function getRegion () {
  const host = window.location.hostname.indexOf('flespi.io') > -1 ? window.location.hostname : (window.location.hostname + ':9005')
  // host = 'localhost:9005'
  let api
  try {
  ...
  const list = []
  let region = ''
  const regions = api.data.result && api.data.result.reduce((a, b) => {
    a[b.name || b.region] = b
...
export default boot(async ({ app, store }) => {
  ...
  let connectionConfig = {
    ...
    socketConfig: { clientId: app, mqttSettings: { protocolVersion: 5, clean: true, wsOptions: { objectMode: false, perMessageDeflate: true }, resubscribe: false, keepalive: 240 } }
  }

  connectionConfig = {
    httpConfig: { server: currentRegion.rest, headers: { 'x-flespi-app': appident } },
    socketConfig: { server: `wss://${currentRegion['mqtt-ws']}`, clientId: appident, mqttSettings: { protocolVersion: 5, clean: true, wsOptions: { objectMode: false, perMessageDeflate: true }, resubscribe: false, keepalive: 240 } }
  }
